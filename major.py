...
if "greeted" not in st.session_state:
    ...
    greet_msg = "Hello Human! I'm your Advance Real-Time Air Quality Tracker. Please enter your city to get the air quality report."
    speak(greet_msg)
    ...
    try:
        ...
        if geo:
            ...
            for idx, pollutant in enumerate(["PM2.5", "PM10", "CO", "NO2"]):
                ...
                prediction_speech_lines.append(summary)
                with cols[idx]:
                    st.markdown(f"""
                        <div class='card'>
            ...
            full_pred_speech = "Forecast and prediction summary: " + " ".join(prediction_speech_lines)
            if st.session_state.get("last_predicted") != city:
                speak(full_pred_speech)
                st.session_state.last_predicted = city
        else:
